...
3. 用户态切换到内核态的方式，（1）系统调用 （2）异步硬件中断。（暂不确定是否还有其它方式），与进程上下文中进入内核代码不同的是，中断上下文无权访问地址空间，因为中断发生时cpu上的进程可能与中断无关
    ...
    `极客时间《深入浅出计算机组成原理》中28篇-异常和中断：程序出错了怎么办？` 论述了异常的分类，与 `Linux Kernel Lab ` 中描述大体相同，只是概念表述上略有不同， Lab 是以 `Interrupt` 为大概念，将 `fault`、 `trap`、`abort` 都归入异常； 而`深计`是以 `Exception` 为大概念，分为 `Interrupt` 、`Trap`、`Fault`、`Abort`四类。虽然分发不同但内容都一样，且同样有同步和异步的描述：
